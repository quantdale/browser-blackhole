# Repository-Local Add-ons Master Plan — browser-blackhole
...
Scientific Three.js/WebGPU/TSL Cosmic Atlas with numerical/CPU-GPU parity tests, Playwright browser/golden suites and explicit scientific-fidelity evidence.

**Decision:** `RECOMMEND_WEBGPU_DIAGNOSTICS_PLUS_DOCS`

No MCP result may be treated as physics validation. Preserve the existing analytic/reference and golden evidence hierarchy.
...
**Why it fits:** Strong fit for WebGPU/WebGL performance, GPU resource, console, memory and frame-time diagnostics on the local atlas.

**Constraints:** Diagnostic only; visual/numerical goldens and benchmark harnesses remain authoritative. Use local targets and disposable profiles.

### 2. Context7 MCP — RECOMMEND

**Type:** MCP

**Why it fits:** Current Three.js/TSL/WebGPU-adjacent library documentation for version-sensitive renderer work.

**Constraints:** Docs only; it cannot establish scientific correctness.

## Explicitly not recommended

- Automatic golden regeneration tools
- Playwright MCP duplicating the existing browser suite
- Any research connector that is allowed to rewrite source provenance automatically
