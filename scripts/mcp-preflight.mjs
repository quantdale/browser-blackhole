...
function loadServers(file) {
  ...
  if (data?.mcpServers && typeof data.mcpServers === 'object') {
    for (const [id, s] of Object.entries(data.mcpServers)) servers.push({ id, s, file });
  }
...
function checkPinned(launcher, args, name, file) {
  if (launcher === 'npx' || launcher === 'uvx') {
    ...
    if (/@latest$/i.test(spec) || spec === 'latest') {
      problems.push(`${file} [${name}]: unpinned "${spec}" — pin an explicit version per plan`);
      return;
    }
    ...
    if (ONLINE) {
      try {
        ...
        problems.push(`${file} [${name}]: pinned package "${spec}" not found on npm registry`);
      }
    }
  }
}
...
for (const c of configs) {
  ...
  for (const { id, s, file } of servers) {
    if (seenIds.has(id)) {
      problems.push(`duplicate server id "${id}" in ${file} and ${seenIds.get(id)}`);
    } else {
      seenIds.set(id, file);
    }
    ...
    if (!ALLOWED_LAUNCHERS.has(launcher)) {
      problems.push(
        `${file} [${id}]: launcher "${launcher}" not in allowed set ${[...ALLOWED_LAUNCHERS].join(
          ','
        )}`
      );
    }
...
console.log(
  `configs scanned: ${[...resolved].map((p) => resolve(p)).join(', ') || '(none found)'}`
);
