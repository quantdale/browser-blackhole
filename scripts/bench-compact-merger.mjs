/**
 * Compact Merger phase-aware performance harness (CA5-15, campaign §23).
 *
 * Measures steady-state FRAME times (rAF deltas — CPU-side, NOT GPU
 * timestamps) of /atlas/compact-merger at a representative timeline position,
 * after pipeline warm-up, at fixed viewport/quality — the same methodology as
 * the other destination harnesses. Workload differs dramatically per phase,
 * so runs must be taken PER PHASE and compared per phase:
 *
 *   node scripts/bench-compact-merger.mjs --phase=0.05  (inspiral)
 *   node scripts/bench-compact-merger.mjs --phase=0.37  (merger)
 *   node scripts/bench-compact-merger.mjs --phase=0.52  (jet)
 *   node scripts/bench-compact-merger.mjs --phase=0.7   (kilonova)
 *   node scripts/bench-compact-merger.mjs --phase=0.9   (afterglow)
 *
 * The record includes the phase's resource state (volume visibility,
 * particle population, jet visibility) so the phase-aware optimization can
 * be verified empirically: inspiral must NOT pay for the volume/particles,
 * non-GRB phases must not pay for jet geometry.
 *
 * Other flags: [--quality=medium|low|high|ultra|auto] [--width=1280]
 * [--height=800] [--frames=480] [--warmup-ms=9000] [--channel=msedge]
 * [--label=cm-run] [--port=4186] [--force-backend=webgpu|webgl2]
 */
...
const preset = String(arg('preset', 'equal-mass-nsns'));
const phase = Math.min(Math.max(Number(arg('phase', '0.7')), 0), 1);
...
const label = String(arg('label', 'cm-run'));
const port = Number(arg('port', '4186'));
...
await page.goto(
  `http://127.0.0.1:${port}/atlas/compact-merger?preset=${encodeURIComponent(preset)}${backendSuffix}`
);
...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    volumeRadius: snap.volumeRadius ?? null,
    populationScale: snap.populationScale ?? null,
    jetVisible: snap.jetVisible ?? null,
    jetFront: snap.jetFrontUnits ?? null,
    ejectaRadius: snap.ejectaRadiusUnits ?? null,
...
const record = {
  schemaVersion: 2,
  kind: 'compact-merger-phase-benchmark',
  date: new Date().toISOString(),
  label,
  commit: currentCommit(),
  destination: 'compact-merger',
  ...
  phaseResources: {
    ...
    jetVisible: info.jetVisible,
    jetFrontUnits: info.jetFront,
    ejectaRadiusUnits: info.ejectaRadius
  },
